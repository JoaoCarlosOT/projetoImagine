<?php if(!defined("BASEPATH")){ exit("No direct script access allowed"); }
         ...
         */
                class Imgno_multiupload extends CI_Upload {
                                ...
                                public function do_multi_upload($field){
                                        ...
                                        for($i=0; $i<count($_FILES[$field]["name"]); $i++){
                                                ...
                                                if(!is_uploaded_file($_FILES[$field]["tmp_name"][$i])){
														...
														echo $error; print_r($_FILES); exit;
                                                        //Set error.
                                                        switch($error){
                                                                //UPLOAD_ERR_INI_SIZE
                                                                case 1:
                        ...
                        public function do_multi_upload_file($file){
                                //Is $_FILES[$field] set? If not, no reason to continue.
                                ...
                                //Every file will have a separate entry in each of the $_FILES associative array elements (name, type, etc).
                                //Loop through $_FILES[$field]["name"] as representative of total number of files. Use count as key in
                                //corresponding elements of the $file elements.
                                for($i=0; $i<count($file["name"]); $i++){
                                        //Was the file able to be uploaded? If not, determine the reason why.
                                        if(!is_uploaded_file($file["tmp_name"][$i])){
                                                //Determine error number.
                                                $error = (!isset($file["error"][$i])) ? 4 : $file["error"][$i];
                                        ...
                                        //Set current file data as class variables.
                                        $this->file_temp        = $file["tmp_name"][$i];
                                        $this->file_size        = $file["size"][$i];
                                        $this->_file_mime_type($file, $i);
                                        $this->file_type        = preg_replace("/^(.+?);.*$/", "\\1", $this->file_type);
                                        $this->file_type        = strtolower(trim(stripslashes($this->file_type), '"'));
                                        $this->file_name        = $this->_prep_filename($file["name"][$i]);
                                        $this->file_ext                = $this->get_extension($this->file_name);
                                        $this->client_name        = $this->file_name;
